...
export async function generateMetadata(props: { params: Promise<{ slug: string }> }) {
  ...
  return generatePageMetadata({ page: category as { title?: string | null; seo?: { title?: string | null; metaDescription?: string | null } | null }, slug: `categories/${slug}`, type: "category" });
}
...
export default async function CategoryPage(props: { params: Promise<{ slug: string }> }) {
  const { slug } = await props.params;
  const category = (await fetchSanityCategoryBySlug({ slug })) as CategoryWithOptionalDescription;
