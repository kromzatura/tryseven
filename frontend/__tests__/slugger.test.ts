...
describe("createUniqueSlugger", () => {
  ...
  it("treats different headings with same base slug as duplicates", () => {
    const unique = createUniqueSlugger();
    expect(unique("The Purity & Quality Advantage — 99.9% Perfection")).toBe(
      "the-purity-quality-advantage-99-9-perfection"
    );
    expect(unique("The Purity & Quality Advantage - 99.9% Perfection")).toBe(
      "the-purity-quality-advantage-99-9-perfection-2"
    );
  });
});
